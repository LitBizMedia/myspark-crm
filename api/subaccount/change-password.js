// api/subaccount/change-password.js
// Authenticated password change for subaccount users.
// Requires a valid session cookie. Verifies current password before
// accepting new one. Stores new password as bcrypt and clears any
// remaining legacy SHA-256 hash.
//
// Audit actions:
//   subaccount.password.change.success
//   subaccount.password.change.failure  - wrong current password or weak new
//   subaccount.password.change.denied   - no session
...
// Server-side password strength check (matches client-side rules)
...
module.exports = async function handler(req, res) {
  ...
  // Break-glass sessions cannot change passwords through this endpoint
  // (they have no DB user record). Direct DB password changes only.
  ...
  const userRes = await fetch(
    SUPABASE_URL + '/rest/v1/subaccount_users?id=eq.' + encodeURIComponent(session.user_id) + '&select=*',
    { headers: sbHeaders() }
  );
  if (!userRes.ok) return res.status(500).json({ error: 'Could not load user' });
  const userRows = await userRes.json();
  if (!userRows || !userRows.length) return res.status(404).json({ error: 'User not found' });
  const user = userRows[0];

  // Verify current password (bcrypt first, fall back to legacy)
  ...
  // Revoke all other sessions for this user (force re-login on other devices)
  // Then issue a fresh session for the current device so this request keeps working
