// api/square/charge.js (Lambda version)
//
// POST /api/square/charge
//
// Charges a card via Web Payments nonce or saved card id.
//
...
async function handler(req, res) {
  ...
  const idempotencyKey = effectiveSlug + '-' + Date.now() + '-' + Math.random().toString(36).slice(2, 10);
  const payload = {
