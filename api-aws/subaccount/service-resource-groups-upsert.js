...
// Replaces ALL resource groups for a service. Atomic per-service operation.
// Body: { service_id, groups: [{ id?, label?, resource_ids: [...] }, ...] }
...
async function handler(req, res) {
  ...
  try {
    ...
    // Verify all referenced resource_ids belong to this subaccount
    ...
    // Delete-and-replace pattern: simpler than diffing groups + members.
    // Cascade handles members via FK ON DELETE CASCADE.
    await db.query(
      `DELETE FROM service_resource_groups WHERE service_id = $1 AND subaccount_id = $2`,
      [serviceId, auth.subaccount_id]
    );

    // Insert fresh groups + members
    for (let i = 0; i < incomingGroups.length; i++) {
      const g = incomingGroups[i];
      const groupId = (g.id && typeof g.id === 'string') ? g.id : uid();
      const label = g.label ? String(g.label).trim() || null : null;

      await db.query(
        `INSERT INTO service_resource_groups (id, service_id, subaccount_id, label, display_order, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`,
        [groupId, serviceId, auth.subaccount_id, label, i]
      );

      // Dedupe resource_ids inside this group
      const seen = {};
      const uniqueIds = [];
      if (Array.isArray(g.resource_ids)) {
        for (const rid of g.resource_ids) {
      ...
      for (const rid of uniqueIds) {
        await db.query(
          `INSERT INTO service_resource_group_members (group_id, resource_id)
           VALUES ($1, $2)
    ...
    await logAudit({
      ...
      metadata: { group_count: incomingGroups.length, resource_count: allResourceIds.length }
    });

    return res.status(200).json({ success: true, group_count: incomingGroups.length });
  } catch (e) {
