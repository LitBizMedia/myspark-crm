// api/subaccount/coupons-toggle.js (Lambda)
// POST /api/subaccount/coupons-toggle
// Body: { id, status }  status in ('active','inactive')
// Admin/manager only (blob migration 2026-06-03).

const { requireSubaccountAuth } = require('./lib/require-subaccount-auth');
const { logAudit } = require('./lib/audit');
const { wrap } = require('./lib/lambda-adapter');
const couponsLib = require('./lib/coupons');

async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const auth = await requireSubaccountAuth(req, res);
  if (!auth) return;

  if (auth.role !== 'admin' && auth.role !== 'super_admin' && auth.role !== 'manager') {
    return res.status(403).json({ error: 'Only admins and managers can manage coupons' });
  }

  const body = req.body || {};
  const id = String(body.id || '').trim();
  if (!id) return res.status(400).json({ error: 'Coupon id is required' });
  const status = body.status === 'inactive' ? 'inactive' : 'active';

  try {
    const coupon = await couponsLib.setCouponStatus(auth.subaccount_id, id, status);
    if (!coupon) return res.status(404).json({ error: 'Coupon not found' });

    await logAudit({
      req,
      actorType: 'subaccount',
      actorId: auth.user_id,
      actorUsername: auth.username,
      actorRole: auth.role,
      action: 'subaccount.coupon.update',
      targetType: 'coupon',
      targetId: id,
      targetSubaccountId: auth.subaccount_id,
      metadata: { status }
    });

    return res.status(200).json({ success: true, coupon });
  } catch (e) {
    console.error('coupons-toggle error:', e.message);
    return res.status(500).json({ error: 'Failed to update coupon status' });
  }
}

exports.handler = wrap(handler);
