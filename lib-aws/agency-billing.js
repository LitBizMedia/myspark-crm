...
// Note: Square nonces are single-use, so non-deterministic key here is acceptable.
...
// Build a deterministic, length-safe idempotency key for Square API calls.
// Square's idempotency_key field is capped at 45 characters. By hashing the
// inputs and prefixing with a short operation tag, we stay well under the
// limit regardless of how long the source data is, and the same inputs always
// produce the same key, so retries dedupe correctly.
//
// Operation codes used in this codebase:
//   'chg' - cron billing charge (input: subaccountId, nextBillingDate, retryCount)
//   'rea' - reactivation charge (input: subaccountId, today)
//   'up'  - upgrade proration charge (input: subaccountId, today, oldTier, newTier)
function makeIdempotencyKey(operation, ...parts) {
  const input = parts.map(p => String(p == null ? '' : p)).join('|');
  const hash = crypto.createHash('sha256').update(input).digest('hex').slice(0, 16);
  return 'msp-' + operation + '-' + hash;
}

// Charge a saved card.
//
// CRITICAL: Pass a deterministic idempotencyKey tied to the logical operation,
// not Date.now(). Square uses idempotency_key to dedupe charges. If the same
// key arrives within 24 hours, Square returns the original payment instead of
// charging again. Without a deterministic key, a network retry or duplicate
// invocation will charge the customer twice.
//
// Recommended key patterns:
//   Cron retry:    'msp-chg-' + subaccountId + '-' + nextBillingDate + '-r' + retryCount
//   Reactivation:  'msp-rea-' + subaccountId + '-' + todayDate
//   Plan upgrade:  'msp-up-'  + subaccountId + '-' + todayDate + '-' + newTier
//
// Returns { success, paymentId, payment } or { success: false, error }.
async function chargeCardOnFile(customerId, cardId, amountCents, note, idempotencyKey) {
  try {
    const creds = await getAgencyCreds();
    const url = 'https://' + squareHost(creds.sandbox) + '/v2/payments';
    if (!idempotencyKey) {
      // Fallback only. Callers should always pass a deterministic key.
      console.warn('chargeCardOnFile called without idempotencyKey. This risks duplicate charges on retry.');
      idempotencyKey = 'msp-chg-' + customerId.slice(-12) + '-' + Date.now().toString().slice(-8);
    }
